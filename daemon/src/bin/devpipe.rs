//! `devpipe` — your Devpipe box, from this machine.
//!
...
//! authenticated WSS on 443 that the web client already uses, against
...
devpipe — your Devpipe box, from this machine
  ...
  devpipe login [--server URL]      sign in once; the token goes to the keychain
  devpipe logout                    end this device's session
  devpipe whoami                    who this machine is signed in as
  devpipe boxes                     what you have, and whether it is awake
  devpipe attach <box> [--new]      attach to a persistent terminal on that box
  devpipe run <box> -- <command>    one command, then exit
  devpipe port <box> <port>         reach that port of the box on localhost
                                  (use 9000:3000 to land on a different one)
  devpipe ls <box> [path]           what is in a directory on the box
  devpipe pull <box>:<path> [dest]  a file or a whole directory, onto this machine
  devpipe push <path> <box>:<path>  the other way
  devpipe edit <box>:<path>         open it in $EDITOR and send back what changed
...
fn main() {
    ...
    let code = match words.as_slice() {
        ...
        ["whoami"] => whoami(),
        ["boxes"] => boxes(),
        ["attach", name, rest @ ..] => attach(name, rest),
        // Compatibility for the pre-rename client. It stays undocumented so
        // new installs converge on the one product verb.
        ["connect", name, rest @ ..] => attach(name, rest),
...
impl Account {
    ...
    fn load() -> Result<Account, String> {
        let stored = keychain::read()
            .ok_or("This machine is not signed in. Run `devpipe login`.".to_string())?;
        ...
        let token = saved
            ...
            .and_then(Value::as_str)
            .map(str::to_string)
            // A bare token is what an older devpipe wrote. Reading it rather than
...
fn finish(req: ureq::Request, body: Option<Value>) -> Result<Value, String> {
    ...
    let agent = format!("devpipe/{} ({})", env!("CARGO_PKG_VERSION"), hostname());
    ...
    match result {
        ...
        Err(ureq::Error::Status(401, _)) => {
            Err("That sign-in has expired. Run `devpipe login`.".into())
        }
...
fn run(name: &str, args: &[&str]) -> i32 {
    ...
    if argv.is_empty() {
        return fail("Nothing to run. Try: devpipe run mybox -- ls ~/work");
    }
    attach_to(name, argv, true)
}

fn attach(name: &str, args: &[&str]) -> i32 {
...
impl Reached {
    ...
    fn sessions(&self) -> Result<Value, String> {
        if self.bare {
            return finish(
                ureq::get(&format!("{}/v1/sessions", self.http()))
                    .set("authorization", &format!("Bearer {}", self.token)),
                None,
            );
        }
        self.account
            .get(&format!("/api/boxes/{}/sessions", self.id))
    }
    ...
    fn create_session(&self, argv: &[String], cols: u16, rows: u16) -> Result<Value, String> {
        ...
        if self.bare {
            return finish(
                ureq::post(&format!("{}/v1/sessions", self.http()))
                    .set("authorization", &format!("Bearer {}", self.token)),
                Some(body),
            );
        }
        self.account
            .post(&format!("/api/boxes/{}/sessions", self.id), body)
    }
...
fn reach(name: &str) -> Result<Reached, String> {
    ...
    //
    //   DEVPIPE_DIRECT=ws://127.0.0.1:7788 DEVPIPE_TOKEN=... devpipe port x 3000
    //
    ...
    if status == "asleep" {
        ...
        return Err(format!(
            "{name} is {status}, not ready. `devpipe boxes` will show what it is doing."
        ));
    }
...
fn wait_until_ready(account: &Account, id: i64) -> Result<(), String> {
    ...
    for _ in 0..80 {
        ...
        let found = list
            ...
            .and_then(|rows| {
                rows.iter()
                    .find(|b| b.get("id").and_then(Value::as_i64) == Some(id))
            })
            .cloned();
...
/// the connection, and an `attach` that started a fresh shell every time would
        ...
        let alive = existing
            ...
            .and_then(|rows| {
                ...
                rows.iter().rfind(|s| {
                    ...
                    // `attach` to somebody's running `claude` would drop the
                    ...
                    // resolves that before it reports it — a plain `attach`
                    // asks for `[]` and is listed back as `["/bin/zsh"]`.
                    // Comparing the two literally never matched, so every
                    // `attach` opened a new shell and the persistence this
        ...
        loop {
            tokio::select! {
                ...
                chunk = typed.recv(), if !stdin_done => match chunk {
                    ...
                    // `devpipe run box -- cmd` looks like in a pipeline and what
        ...
        while let Some(Ok(message)) = rx.next().await {
            match message {
                ...
                Message::Text(text) => {
                    ...
                    {
                        eprintln!("devpipe: nothing is listening on the box: {why}");
                        break;
                    }
                }
...
mod keychain {
    ...
    const ACCOUNT: &str = "devpipe";
    const LEGACY_ACCOUNT: &str = "dpctl";
...
/// one `attach` and `port` use — rather than reopening SSH, which a box locks
...
fn ls(name: &str, path: Option<&str>) -> i32 {
    ...
    println!(
        "{}",
        listing.get("path").and_then(Value::as_str).unwrap_or("")
    );
...
fn pull(spec: &str, dest: Option<&str>) -> i32 {
    let Some((name, remote)) = split_remote(spec) else {
        return fail("A pull looks like `devpipe pull mybox:~/notes.md .`");
    };
    let reached = match reach(name) {
        Ok(r) => r,
        Err(e) => return fail(&e),
...
fn push(local: &str, spec: &str) -> i32 {
    let Some((name, remote)) = split_remote(spec) else {
        return fail("A push looks like `devpipe push ./notes.md mybox:~/notes.md`");
    };
...
fn edit(spec: &str) -> i32 {
    let Some((name, remote)) = split_remote(spec) else {
        return fail("An edit looks like `devpipe edit mybox:~/src/main.rs`");
    };
    let reached = match reach(name) {
        Ok(r) => r,
        Err(e) => return fail(&e),
    ...
    let leaf = remote
        ...
        .next()
        .unwrap_or("file");
    let scratch = std::env::temp_dir().join(format!("devpipe-{}-{leaf}", std::process::id()));
