...
async fn child_output_reaches_an_attached_client() {
    ...
    let id = server
        .create(&["/bin/sh", "-c", "echo marker-alpha; sleep 30"], 80, 24)
        .await;
...
async fn resize_is_visible_to_the_child() {
    ...
    ws.send(Message::Text(
        r#"{"t":"resize","cols":120,"rows":40}"#.into(),
    ))
    .await
    .unwrap();
...
async fn output_while_detached_is_not_lost() {
    ...
    let id = server
        .create(
            &["/bin/sh", "-c", "sleep 1; echo marker-echo; sleep 30"],
            80,
            24,
        )
        .await;
...
async fn ctrl_c_still_interrupts_after_a_resize() {
    ...
    let mut ws = server.attach(&id).await;
    ws.send(Message::Text(
        r#"{"t":"resize","cols":69,"rows":70}"#.into(),
    ))
    .await
    .unwrap();
    tokio::time::sleep(Duration::from_millis(300)).await;
