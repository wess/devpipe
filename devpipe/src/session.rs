...
//! socket exists — attaching is a subscription, detaching drops it, and the
//! child never notices either. That is the whole product promise, and it falls
//! out of keeping the pty thread independent of the transport.
...
pub struct Session {
    ...
    /// Input the pty has not accepted yet. The pty thread blocks in `poll`, so
    /// a writer appends here and rings the waker.
    pending: Arc<Mutex<VecDeque<u8>>>,
    waker: Waker,
    control: Control,
    mirror: Arc<Mutex<Terminal>>,
    /// A watch rather than a flag: a pane pump has to *wait* for the child to
    /// die, and polling a bool is the version of this that either burns a core
    /// or reports the exit late.
    alive: Arc<watch::Sender<bool>>,
    shutdown: Arc<watch::Sender<bool>>,
}

impl Session {
    /// `entry` is the backend's answer to "run this here": an argv on this
    /// machine, the environment to run it with, and where. Nothing below this
    /// line knows whether that lands in a container or on the host.
    pub fn spawn(id: String, entry: Entry, cols: u16, rows: u16) -> io::Result<Arc<Session>> {
        let Entry { argv, env, cwd } = entry;
        ...
        opts.cwd = cwd;
        // TERM decides what the child believes it can emit. Every client we
        // ship runs sinclair's vt, which handles the 256-color and truecolor
        // paths, so claiming xterm-256color is honest rather than optimistic.
        ...
        let alive = Arc::new(watch::channel(true).0);
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let _ = child.kill();
                let _ = child.wait();
                // Last, so nothing wakes on the exit and then races the reap
                // against a pty that is still closing.
    ...
    /// Resolves when the child has gone. Already-exited is not a missed edge:
    /// the watch holds the state rather than signalling the transition.
